...
class LeaveSettlement(Document):
	...
	if TYPE_CHECKING:
		...
		from orion_erp.doctype.gratuity_pay.gratuity_pay import GratuityPay
		from orion_erp.doctype.leave_pay.leave_pay import LeavePay
		from orion_erp.doctype.leave_settlement_deductions.leave_settlement_deductions import LeaveSettlementDeductions
		from orion_erp.doctype.salary_due.salary_due import SalaryDue
		from orion_erp.doctype.ticket_allowance.ticket_allowance import TicketAllowance
...
def revert_ticket_paid(doc, method=None):
	...
	for row in doc.ticket_allowance:
		...
		for child in employee_doc.custom_ticket_allowance_detail:

			if (
				str(child.from_date) == str(row.get("from"))
				and
				str(child.to_date) == str(row.get("to"))
			):
				...
				# existing html
				reference_html = (
					child.references_data or ""
				)

				# remove current settlement row
				pattern = rf"""
					<tr>\s*
						<td>\s*
							<a[^>]*>\s*
				...
				# update values
				child.paid_amount = total_paid_amount

				child.outstanding_amount = outstanding_amount

				child.references_data = reference_html
				...
				if (
					...
					child.paid = 1
					child.partial_paid = 0

				elif total_paid_amount > 0:

					child.paid = 0
					child.partial_paid = 1
