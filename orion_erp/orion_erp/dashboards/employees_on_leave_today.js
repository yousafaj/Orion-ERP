frappe.dom.freeze("Loading Employees on Leave...");

async function load_employees_on_leave_today() {
    root_element.innerHTML = "";

    try {
        const { message: data } = await frappe.call({
            method: "orion_erp.api.get_employees_on_leave_today"
        });
...
<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:300px;padding:60px 40px;background:#fff;border-radius:12px;box-shadow:0 2px 12px rgba(0,0,0,.06);font-family:Inter,system-ui,sans-serif">
    ...
    <h3 style="margin:0 0 8px;color:#374151;font-weight:600;font-size:20px">No Employees on Leave Today</h3>
    <p style="margin:0;color:#9ca3af;font-size:14px">All employees are present today.</p>
</div>`;
            frappe.dom.unfreeze();
            return;
        }
...
<span style="display:inline-block;padding:3px 10px;border-radius:20px;font-size:11px;font-weight:600;color:#16a34a;background:#f0fdf4;border:1px solid #bbf7d0">Approved</span>
</td></tr>`;
        });
...
<div class="dashboard-body">
    <div style="background:linear-gradient(135deg,#1e40af,#2563eb,#3b82f6);border-radius:12px 12px 0 0;padding:12px 20px;display:flex;align-items:center;gap:12px;box-shadow:0 4px 16px rgba(37,99,235,0.25)">
        ...
        <div style="flex:1">
            <div style="font-size:16px;font-weight:700;color:#fff;letter-spacing:-0.3px">Employees on Leave Today</div>
            <div style="font-size:12px;color:rgba(255,255,255,0.75);margin-top:0">Currently on Approved Leave</div>
        </div>
    ...
    <div class="kpi-grid">
        <div class="kpi-card" style="display:flex;align-items:center;gap:12px;padding:10px 14px;background:#f0fdf4;border-radius:10px;border:1px solid #bbf7d0;transition:all 0.25s">
            ...
            <div>
                <div style="font-size:10px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:1px">On Leave Today</div>
                <div style="font-size:22px;font-weight:800;color:#15803d;line-height:1">${kpi.employees_on_leave}</div>
        ...
        <div class="kpi-card" style="display:flex;align-items:center;gap:12px;padding:10px 14px;background:#eff6ff;border-radius:10px;border:1px solid #bfdbfe;transition:all 0.25s">
            <div style="display:flex;align-items:center;justify-content:center;width:36px;height:36px;border-radius:8px;background:linear-gradient(135deg,#dbeafe,#eff6ff);color:#2563eb;font-size:15px;flex-shrink:0"><i class="fa fa-building"></i></div>
            <div>
                <div style="font-size:10px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:1px">Departments</div>
                <div style="font-size:22px;font-weight:800;color:#1d4ed8;line-height:1">${kpi.departments_impacted}</div>
                <div style="font-size:11px;color:#6b7280;margin-top:0">Impacted</div>
            </div>
        </div>
        <div class="kpi-card" style="display:flex;align-items:center;gap:12px;padding:10px 14px;background:#fffbeb;border-radius:10px;border:1px solid #fde68a;transition:all 0.25s">
            <div style="display:flex;align-items:center;justify-content:center;width:36px;height:36px;border-radius:8px;background:linear-gradient(135deg,#fef3c7,#fffbeb);color:#f59e0b;font-size:15px;flex-shrink:0"><i class="fa fa-tags"></i></div>
            <div>
                <div style="font-size:10px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:1px">Leave Types</div>
                <div style="font-size:22px;font-weight:800;color:#d97706;line-height:1">${kpi.leave_types_count}</div>
                <div style="font-size:11px;color:#6b7280;margin-top:0">Used Today</div>
            </div>
        </div>
        <div class="kpi-card" style="display:flex;align-items:center;gap:12px;padding:10px 14px;background:#fafafa;border-radius:10px;border:1px solid #e5e7eb;transition:all 0.25s">
            ...
            <div>
                <div style="font-size:10px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:1px">Today's Date</div>
                <div style="font-size:13px;font-weight:700;color:#1f2937;line-height:1;white-space:nowrap;margin-bottom:1px">${frappe.datetime.str_to_user(kpi.today_date)}</div>
                <div style="font-size:11px;color:#6b7280;margin-top:0">Current Date</div>
            </div>
        </div>
    </div>
    ...
    <div style="background:#fff;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 12px 12px;overflow:hidden;box-shadow:0 4px 12px rgba(0,0,0,.04)">
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;padding:10px 16px;background:#f8fafc;border-bottom:1px solid #e5e7eb">
            <div style="font-size:13px;font-weight:700;color:#374151;display:flex;align-items:center;gap:6px;margin-right:auto">
                <i class="fa fa-list-alt" style="color:#2563eb;font-size:12px"></i> Employees on Leave Today
        ...
        <div style="padding:6px 16px;background:#f8fafc;border-top:1px solid #e5e7eb;display:flex;justify-content:space-between;align-items:center;font-size:11px;color:#6b7280">
            <span>Showing ${total_rows} employee${total_rows !== 1 ? 's' : ''} on leave today</span>
        ...
        $("#refreshBtn", root_element).on("click", function (e) { e.stopPropagation(); load_employees_on_leave_today(); });
        $("#openLeaveListBtn", root_element).on("click", function (e) { e.stopPropagation(); frappe.set_route("List", "Leave Application"); });
        ...
        $("#exportBtn", root_element).on("click", function () {
            ...
            const csv = [];
            csv.push('"EMPLOYEES ON LEAVE TODAY"');
            csv.push('');
            csv.push('"Export Date","' + frappe.datetime.str_to_user(today) + '"');
            csv.push('"Department","' + dept + '"');
            csv.push('"Leave Type","' + ltype + '"');
            csv.push('');
            csv.push('"On Leave","' + kpi.employees_on_leave + '","Departments","' + kpi.departments_impacted + '"');
            csv.push('');
            csv.push('');
            const headers = [];
            ...
            link.download = "Employees_On_Leave_Today_" + today + ".csv";
...
<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:300px;padding:60px 40px;background:#fff;border-radius:12px;box-shadow:0 2px 12px rgba(0,0,0,.06);font-family:Inter,system-ui,sans-serif">
    ...
    <button id="retryBtn" style="padding:10px 24px;border:none;border-radius:8px;background:#ef4444;color:#fff;font-size:14px;font-weight:600;cursor:pointer;font-family:inherit">Try Again</button>
        ...
        $("#retryBtn", root_element).on("click", function (e) { e.stopPropagation(); load_employees_on_leave_today(); });
    } finally {
        frappe.dom.unfreeze();
    }
}

load_employees_on_leave_today();
