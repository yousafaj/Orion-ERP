    ...
    <p style="margin:0;color:#9ca3af;font-size:14px">All your leave applications have been processed.</p>
        ...
        leave_data.forEach((row, idx) => {
            ...
            table_rows += [
                ...
                '<td style="padding:8px 16px;font-size:12px;font-weight:500;color:#2563eb;border-bottom:1px solid #f0f0f0;cursor:pointer" class="app-link" data-name="', frappe.utils.escape_html(row.name), '">',
                frappe.utils.escape_html(row.name), '</td>',
                '<td style="padding:8px 16px;font-size:13px;font-weight:500;color:#1f2937;border-bottom:1px solid #f0f0f0">', escaped_type, '</td>',
                '<td style="padding:8px 16px;font-size:12px;color:#6b7280;border-bottom:1px solid #f0f0f0;white-space:nowrap">', frappe.datetime.str_to_user(row.from_date), '</td>',
                '<td style="padding:8px 16px;font-size:12px;color:#6b7280;border-bottom:1px solid #f0f0f0;white-space:nowrap">', frappe.datetime.str_to_user(row.to_date), '</td>',
                ...
                '<span style="display:inline-block;padding:3px 10px;border-radius:20px;font-size:11px;font-weight:600;color:', status_badge_color, ';background:', status_bg, ';border:1px solid ', status_badge_color, '33">',
...
<div class="dashboard-body">
    <div style="background:linear-gradient(135deg,#1e40af,#2563eb,#3b82f6);border-radius:12px 12px 0 0;padding:12px 20px;display:flex;align-items:center;gap:12px;box-shadow:0 4px 16px rgba(37,99,235,0.25)">
        ...
        <div style="flex:1">
            <div style="font-size:16px;font-weight:700;color:#fff;letter-spacing:-0.3px">Pending Leave Applications</div>
            <div style="font-size:12px;color:rgba(255,255,255,0.75);margin-top:0">Applications Awaiting Approval</div>
        </div>
    ...
    <div class="kpi-grid">
        ...
        <div class="kpi-card" style="display:flex;align-items:center;gap:12px;padding:10px 14px;background:#fffbeb;border-radius:10px;border:1px solid #fde68a;transition:all 0.25s">
            ...
            <div>
                ...
                <div style="font-size:22px;font-weight:800;color:#d97706;line-height:1">${pending_count}</div>
        ...
        <div class="kpi-card" style="display:flex;align-items:center;gap:12px;padding:10px 14px;background:#f0fdf4;border-radius:10px;border:1px solid #bbf7d0;transition:all 0.25s">
            <div style="display:flex;align-items:center;justify-content:center;width:36px;height:36px;border-radius:8px;background:linear-gradient(135deg,#dcfce7,#f0fdf4);color:#16a34a;font-size:15px;flex-shrink:0"><i class="fa fa-calculator"></i></div>
            <div>
                <div style="font-size:10px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:1px">Total Requested Days</div>
                <div style="font-size:22px;font-weight:800;color:#15803d;line-height:1">${total_requested_days}</div>
                <div style="font-size:11px;color:#6b7280;margin-top:0">Days Pending</div>
            </div>
        </div>
        ...
        <div class="kpi-card" style="display:flex;align-items:center;gap:12px;padding:10px 14px;background:#eff6ff;border-radius:10px;border:1px solid #bfdbfe;transition:all 0.25s">
            <div style="display:flex;align-items:center;justify-content:center;width:36px;height:36px;border-radius:8px;background:linear-gradient(135deg,#dbeafe,#eff6ff);color:#2563eb;font-size:15px;flex-shrink:0"><i class="fa fa-calendar-check-o"></i></div>
            <div>
                <div style="font-size:10px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:1px">Latest Request</div>
                <div style="font-size:12px;font-weight:700;color:#1d4ed8;line-height:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin-bottom:1px">${latest_request || "N/A"}</div>
                <div style="font-size:11px;color:#6b7280;margin-top:0">Most Recent</div>
            </div>
        </div>
    </div>
    ...
    <div style="background:#fff;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 12px 12px;overflow:hidden;box-shadow:0 4px 12px rgba(0,0,0,.04)">
        <div style="display:flex;align-items:center;padding:10px 16px;background:#f8fafc;border-bottom:1px solid #e5e7eb">
            <div style="font-size:13px;font-weight:700;color:#374151;display:flex;align-items:center;gap:6px">
                ...
                <span style="font-size:11px;font-weight:500;color:#6b7280;background:#e5e7eb;padding:1px 8px;border-radius:10px;margin-left:4px">${pending_count}</span>
            </div>
        </div>
            ...
            <table style="width:100%;border-collapse:collapse;font-size:13px">
                <thead>
                    <tr style="background:#f1f5f9;position:sticky;top:0;z-index:10">
                        ...
                        <th style="padding:10px 16px;text-align:center;font-size:11px;font-weight:700;color:#374151;text-transform:uppercase;letter-spacing:0.5px;border-bottom:2px solid #e2e8f0;width:120px">Status</th>
                    </tr>
                </thead>
