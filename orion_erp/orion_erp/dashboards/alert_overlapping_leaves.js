...
<td style="padding:14px 20px;font-size:13px;color:#6b7280;border-bottom:1px solid #f0f0f0;white-space:nowrap">${r.date}</td>
...
<td style="padding:14px 20px;font-size:13px;color:#1f2937;border-bottom:1px solid #f0f0f0;font-weight:500">${frappe.utils.escape_html(r.employee_names)}</td>
<td style="padding:14px 20px;text-align:center;font-size:14px;font-weight:600;color:#1f2937;border-bottom:1px solid #f0f0f0">${r.count}</td>
<td style="padding:14px 20px;font-size:13px;color:#6b7280;border-bottom:1px solid #f0f0f0">${frappe.utils.escape_html(r.leave_types)}</td>
<td style="padding:14px 20px;text-align:center;border-bottom:1px solid #f0f0f0">
<span style="display:inline-block;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600;color:${r.severity==="Critical"?"#dc2626":r.severity==="High"?"#d97706":"#6b7280"};background:${r.severity==="Critical"?"#fee2e2":r.severity==="High"?"#fef3c7":"#f3f4f6"}">${r.severity}</span>
</td>
<td style="padding:14px 20px;font-size:13px;border-bottom:1px solid #f0f0f0">${applicationLinks}</td>
</tr>`;
    });
...
<div class="dashboard-body">
    <div style="background:linear-gradient(135deg,#1e40af,#2563eb,#3b82f6);border-radius:12px 12px 0 0;padding:20px 28px;display:flex;align-items:center;gap:16px;box-shadow:0 4px 16px rgba(37,99,235,0.25)">
        ...
        <div style="flex:1">
            <div style="font-size:20px;font-weight:700;color:#fff;letter-spacing:-0.3px">Team Leave Overlap Alerts</div>
            <div style="font-size:13px;color:rgba(255,255,255,0.75);margin-top:2px">Detected Overlapping Leave Dates</div>
        </div>
    ...
    <div class="kpi-grid">
        <div class="kpi-card" style="display:flex;align-items:center;gap:16px;padding:18px 20px;background:#fef2f2;border-radius:10px;border:1px solid #fecaca;transition:all 0.25s">
            <div style="display:flex;align-items:center;justify-content:center;width:46px;height:46px;border-radius:10px;background:linear-gradient(135deg,#fee2e2,#fef2f2);color:#ef4444;font-size:20px;flex-shrink:0"><i class="fa fa-exclamation-circle"></i></div>
            <div>
                <div style="font-size:11px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:2px">Critical</div>
                <div style="font-size:28px;font-weight:800;color:#dc2626;line-height:1.1">${critical}</div>
                <div style="font-size:12px;color:#6b7280;margin-top:2px">4+ Employees Overlapping</div>
            </div>
        </div>
        <div class="kpi-card" style="display:flex;align-items:center;gap:16px;padding:18px 20px;background:#fffbeb;border-radius:10px;border:1px solid #fde68a;transition:all 0.25s">
            <div style="display:flex;align-items:center;justify-content:center;width:46px;height:46px;border-radius:10px;background:linear-gradient(135deg,#fef3c7,#fffbeb);color:#f59e0b;font-size:20px;flex-shrink:0"><i class="fa fa-calendar"></i></div>
            <div>
                <div style="font-size:11px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:2px">Upcoming Conflicts</div>
                <div style="font-size:28px;font-weight:800;color:#d97706;line-height:1.1">${upcoming}</div>
                <div style="font-size:12px;color:#6b7280;margin-top:2px">Future Overlaps</div>
            </div>
        </div>
        <div class="kpi-card" style="display:flex;align-items:center;gap:16px;padding:18px 20px;background:#f0fdf4;border-radius:10px;border:1px solid #bbf7d0;transition:all 0.25s">
            <div style="display:flex;align-items:center;justify-content:center;width:46px;height:46px;border-radius:10px;background:linear-gradient(135deg,#dcfce7,#f0fdf4);color:#16a34a;font-size:20px;flex-shrink:0"><i class="fa fa-calendar-check-o"></i></div>
            <div>
                <div style="font-size:11px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:2px">Today's Conflicts</div>
                <div style="font-size:28px;font-weight:800;color:#15803d;line-height:1.1">${todayCnt}</div>
                <div style="font-size:12px;color:#6b7280;margin-top:2px">Active Overlaps</div>
            </div>
        </div>
    </div>
    ...
    <div style="background:#fff;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 12px 12px;overflow:hidden;box-shadow:0 4px 12px rgba(0,0,0,.04)">
        <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;padding:14px 20px;background:#f8fafc;border-bottom:1px solid #e5e7eb">
            <div style="font-size:14px;font-weight:700;color:#374151;display:flex;align-items:center;gap:8px;margin-right:auto">
                <i class="fa fa-list-alt" style="color:#2563eb;font-size:13px"></i> Overlap Details
                <span style="font-size:12px;font-weight:500;color:#6b7280;background:#e5e7eb;padding:2px 10px;border-radius:12px;margin-left:4px">${rows.length}</span>
            </div>
            ...
            <table style="width:100%;border-collapse:collapse;font-size:14px">
                <thead>
                    <tr style="background:#f1f5f9;position:sticky;top:0;z-index:10">
                        <th style="padding:12px 20px;text-align:left;font-size:12px;font-weight:700;color:#374151;text-transform:uppercase;letter-spacing:0.5px;border-bottom:2px solid #e2e8f0">Date</th>
                        ...
                        <th style="padding:12px 20px;text-align:left;font-size:12px;font-weight:700;color:#374151;text-transform:uppercase;letter-spacing:0.5px;border-bottom:2px solid #e2e8f0">Employees</th>
                        <th style="padding:12px 20px;text-align:center;font-size:12px;font-weight:700;color:#374151;text-transform:uppercase;letter-spacing:0.5px;border-bottom:2px solid #e2e8f0;width:60px">Count</th>
                        ...
                        <th style="padding:12px 20px;text-align:center;font-size:12px;font-weight:700;color:#374151;text-transform:uppercase;letter-spacing:0.5px;border-bottom:2px solid #e2e8f0;width:80px">Severity</th>
                        <th style="padding:12px 20px;text-align:left;font-size:12px;font-weight:700;color:#374151;text-transform:uppercase;letter-spacing:0.5px;border-bottom:2px solid #e2e8f0">Applications</th>
                    </tr>
                </thead>
