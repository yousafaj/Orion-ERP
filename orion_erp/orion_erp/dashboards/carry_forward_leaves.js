...
async function load_carry_forward() {
    ...
    try {
        ...
        const { message: cf_data } = await frappe.call({
    ...
    <p style="margin:0;color:#9ca3af;font-size:14px">You have no carry-forward leave from the previous year.</p>
        ...
        cf_data.forEach((row, idx) => {
            ...
            table_rows += [
                ...
                '<span style="display:inline-block;padding:4px 16px;border-radius:20px;font-size:13px;font-weight:700;color:#fff;background:linear-gradient(135deg,#f97316,#ea580c);box-shadow:0 2px 6px rgba(249,115,22,0.25);min-width:60px">',
                days, '</span></td>',
                '</tr>'
            ].join('');
        });
...
<div class="dashboard-body">
    <div style="background:linear-gradient(135deg,#1e40af,#2563eb,#3b82f6);border-radius:12px 12px 0 0;padding:20px 28px;display:flex;align-items:center;gap:16px;box-shadow:0 4px 16px rgba(37,99,235,0.25)">
        ...
        <div style="flex:1">
            <div style="font-size:20px;font-weight:700;color:#fff;letter-spacing:-0.3px">Carry Forward Leave Summary</div>
            <div style="font-size:13px;color:rgba(255,255,255,0.75);margin-top:2px">Leave Brought Forward</div>
        </div>
    ...
    <div class="kpi-grid">
        ...
        <div class="kpi-card" style="display:flex;align-items:center;gap:16px;padding:18px 20px;background:#fffbeb;border-radius:10px;border:1px solid #fde68a;transition:all 0.25s">
            ...
            <div>
                <div style="font-size:11px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:2px">Previous Year</div>
                <div style="font-size:15px;font-weight:700;color:#d97706;line-height:1.1">${prev_year}</div>
                <div style="font-size:12px;color:#6b7280;margin-top:2px">Carry Forward Period</div>
            </div>
        </div>
        ...
        <div class="kpi-card" style="display:flex;align-items:center;gap:16px;padding:18px 20px;background:#f0fdf4;border-radius:10px;border:1px solid #bbf7d0;transition:all 0.25s">
            ...
            <div>
                <div style="font-size:11px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:2px">Types Carried</div>
                <div style="font-size:28px;font-weight:800;color:#15803d;line-height:1.1">${total_types}</div>
                <div style="font-size:12px;color:#6b7280;margin-top:2px">Leave Types</div>
            </div>
        </div>

        <div class="kpi-card" style="display:flex;align-items:center;gap:16px;padding:18px 20px;background:#fffbeb;border-radius:10px;border:1px solid #fde68a;transition:all 0.25s">
            <div style="display:flex;align-items:center;justify-content:center;width:46px;height:46px;border-radius:10px;background:linear-gradient(135deg,#fef3c7,#fffbeb);color:#f59e0b;font-size:20px;flex-shrink:0"><i class="fa fa-calculator"></i></div>
            <div>
                <div style="font-size:11px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:2px">Total CF Days</div>
                <div style="font-size:28px;font-weight:800;color:#d97706;line-height:1.1">${total_cf}</div>
                <div style="font-size:12px;color:#6b7280;margin-top:2px">Carried Forward</div>
            </div>
        </div>
    </div>
    ...
    <div style="background:#fff;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 12px 12px;overflow:hidden;box-shadow:0 4px 12px rgba(0,0,0,.04)">
        <div style="display:flex;align-items:center;padding:14px 20px;background:#f8fafc;border-bottom:1px solid #e5e7eb">
            <div style="font-size:14px;font-weight:700;color:#374151;display:flex;align-items:center;gap:8px">
                <i class="fa fa-list-alt" style="color:#2563eb;font-size:13px"></i> Carry Forward Details
            ...
            <table style="width:100%;border-collapse:collapse;font-size:14px">
                <thead>
                    <tr style="background:#f1f5f9;position:sticky;top:0;z-index:10">
                        ...
                        <th style="padding:12px 20px;text-align:center;font-size:12px;font-weight:700;color:#374151;text-transform:uppercase;letter-spacing:0.5px;border-bottom:2px solid #e2e8f0;width:180px">Carry Forward Days</th>
                    </tr>
                </thead>
