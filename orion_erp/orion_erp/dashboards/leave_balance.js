frappe.dom.freeze("Loading Leave Balance...");

async function load_leave_balance() {
    ...
    try {
        ...
        const { message: leave_data } = await frappe.call({
            method: "orion_erp.api.get_employee_leave_balances"
        });
        ...
        if (!leave_data || !leave_data.length) {
            root_element.innerHTML = [
                '<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:300px;padding:60px 40px;background:#fff;border-radius:12px;box-shadow:0 2px 12px rgba(0,0,0,.06);font-family:Inter,system-ui,sans-serif">',
                '<div style="font-size:64px;margin-bottom:20px;opacity:0.3">&#x1F4CB;</div>',
                '<h3 style="margin:0 0 8px;color:#374151;font-weight:600;font-size:20px">No Leave Balance Found</h3>',
                '<p style="margin:0;color:#9ca3af;font-size:14px">You currently have no active leave allocations.</p>',
                '</div>'
            ].join('');
            frappe.dom.unfreeze();
            return;
        ...
        let total_balance = 0;
        let total_types = leave_data.length;
        ...
        leave_data.forEach((row, idx) => {
            ...
            const to_date = row.to_date ? frappe.datetime.str_to_user(row.to_date) : '—';
            table_rows += [
                '<tr style="background:', (idx % 2 === 0 ? '#ffffff' : '#f8fafc'), '">',
                '<td style="padding:8px 16px;font-size:13px;font-weight:500;color:#1f2937;border-bottom:1px solid #f0f0f0">',
                escaped_type,
                '</td>',
                '<td style="padding:8px 16px;text-align:center;font-size:12px;color:#6b7280;border-bottom:1px solid #f0f0f0;white-space:nowrap">',
                from_date, ' — ', to_date,
                '</td>',
                '<td style="padding:8px 16px;text-align:center;border-bottom:1px solid #f0f0f0">',
                '<span style="display:inline-block;padding:3px 12px;border-radius:20px;font-size:12px;font-weight:700;color:#fff;background:',
                (balance > 0 ? 'linear-gradient(135deg,#16a34a,#15803d)' : '#9ca3af'),
                ';box-shadow:0 2px 6px rgba(22,163,74,0.25);min-width:50px">',
        ...
        root_element.innerHTML = [
            ...
            '<div style="flex:1">',
            '<div style="font-size:16px;font-weight:700;color:#fff;letter-spacing:-0.3px">My Leave Balance</div>',
            '<div style="font-size:12px;color:rgba(255,255,255,0.75);margin-top:0">Employee Leave Summary Dashboard</div>',
            '</div>',
            ...
            '<div style="display:flex;align-items:center;gap:12px;padding:10px 14px;background:#f0fdf4;border-radius:10px;border:1px solid #bbf7d0;transition:all 0.25s" class="kpi-card">',
            ...
            '<div><div style="font-size:10px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:1px">Leave Types</div>',
            '<div style="font-size:22px;font-weight:800;color:#15803d;line-height:1">', total_types, '</div>',
            '<div style="font-size:11px;color:#6b7280;margin-top:0">Active Allocations</div></div></div>',

            '<div style="display:flex;align-items:center;gap:12px;padding:10px 14px;background:#eff6ff;border-radius:10px;border:1px solid #bfdbfe;transition:all 0.25s" class="kpi-card">',
            ...
            '<div><div style="font-size:10px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:1px">Total Balance</div>',
            '<div style="font-size:22px;font-weight:800;color:#1d4ed8;line-height:1">', total_balance, '</div>',
            '<div style="font-size:11px;color:#6b7280;margin-top:0">Days Available</div></div></div>',

            '<div style="display:flex;align-items:center;gap:12px;padding:10px 14px;background:#fafafa;border-radius:10px;border:1px solid #e5e7eb;transition:all 0.25s" class="kpi-card">',
            '<div style="display:flex;align-items:center;justify-content:center;width:36px;height:36px;border-radius:8px;background:linear-gradient(135deg,#f3f4f6,#f9fafb);color:#6b7280;font-size:15px;flex-shrink:0"><i class="fa fa-calendar"></i></div>',
            '<div><div style="font-size:10px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:1px">As of Date</div>',
            '<div style="font-size:14px;font-weight:700;color:#1f2937;line-height:1;white-space:nowrap;margin-bottom:1px">', today, '</div>',
            '<div style="font-size:11px;color:#6b7280;margin-top:0">Last Updated</div></div></div></div>',
            ...
            '<span style="font-size:11px;font-weight:500;color:#6b7280;background:#e5e7eb;padding:1px 8px;border-radius:10px;margin-left:8px">', total_types, '</span></div></div>',
            ...
            '<thead><tr style="background:#f1f5f9;position:sticky;top:0;z-index:10">',
             ...
             '<th style="padding:10px 16px;text-align:center;font-size:11px;font-weight:700;color:#374151;text-transform:uppercase;letter-spacing:0.5px;border-bottom:2px solid #e2e8f0">Allocation Period</th>',
             '<th style="padding:10px 16px;text-align:center;font-size:11px;font-weight:700;color:#374151;text-transform:uppercase;letter-spacing:0.5px;border-bottom:2px solid #e2e8f0;width:150px">Available Days</th>',
            '</tr></thead>',
        ...
        // Stop propagation so widget click handlers don't intercept
        // Bind events AFTER rendering - same pattern as working blocks
        $("#openLeaveBtn", root_element).on("click", function (e) {
            e.stopPropagation();
