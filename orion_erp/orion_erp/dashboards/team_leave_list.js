...
<td style="padding:14px 20px;font-size:14px;font-weight:500;color:#1f2937;border-bottom:1px solid #f0f0f0"><b>${frappe.utils.escape_html(r.employee_name||"")}</b><br><span style="color:#64748b;font-size:12px;white-space:nowrap">${frappe.utils.escape_html(r.employee||"")}</span></td>
...
<span style="display:inline-block;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600;color:#fff;background:${r.status_bg}">${r.status_text}</span>
</td>
</tr>`;
    });
...
<div class="dashboard-body">
    <div style="background:linear-gradient(135deg,#1e40af,#2563eb,#3b82f6);border-radius:12px 12px 0 0;padding:20px 28px;display:flex;align-items:center;gap:16px;box-shadow:0 4px 16px rgba(37,99,235,0.25)">
        ...
        <div style="flex:1">
            <div style="font-size:20px;font-weight:700;color:#fff;letter-spacing:-0.3px">Team Leave Schedule</div>
            <div style="font-size:13px;color:rgba(255,255,255,0.75);margin-top:2px">Employee Leave Overview</div>
        </div>
    ...
    <div class="kpi-grid">
        <div class="kpi-card" style="display:flex;align-items:center;gap:16px;padding:18px 20px;background:#f0fdf4;border-radius:10px;border:1px solid #bbf7d0;transition:all 0.25s">
            <div style="display:flex;align-items:center;justify-content:center;width:46px;height:46px;border-radius:10px;background:linear-gradient(135deg,#dcfce7,#f0fdf4);color:#16a34a;font-size:20px;flex-shrink:0"><i class="fa fa-users"></i></div>
            <div>
                <div style="font-size:11px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:2px">On Leave</div>
                <div style="font-size:28px;font-weight:800;color:#15803d;line-height:1.1">${on_leave}</div>
                <div style="font-size:12px;color:#6b7280;margin-top:2px">Currently Away</div>
            </div>
        </div>
        <div class="kpi-card" style="display:flex;align-items:center;gap:16px;padding:18px 20px;background:#eff6ff;border-radius:10px;border:1px solid #bfdbfe;transition:all 0.25s">
            ...
            <div>
                <div style="font-size:11px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:2px">Upcoming</div>
                <div style="font-size:28px;font-weight:800;color:#1d4ed8;line-height:1.1">${upcoming}</div>
                <div style="font-size:12px;color:#6b7280;margin-top:2px">Scheduled Leaves</div>
            </div>
        </div>
        <div class="kpi-card" style="display:flex;align-items:center;gap:16px;padding:18px 20px;background:#f8fafc;border-radius:10px;border:1px solid #e5e7eb;transition:all 0.25s">
            <div style="display:flex;align-items:center;justify-content:center;width:46px;height:46px;border-radius:10px;background:linear-gradient(135deg,#dbeafe,#eff6ff);color:#2563eb;font-size:20px;flex-shrink:0"><i class="fa fa-list"></i></div>
            <div>
                <div style="font-size:11px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:2px">Total Records</div>
                <div style="font-size:28px;font-weight:800;color:#1d4ed8;line-height:1.1">${data.length}</div>
                <div style="font-size:12px;color:#6b7280;margin-top:2px">Active & Upcoming</div>
            </div>
        </div>
    </div>
    ...
    <div style="background:#fff;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 12px 12px;overflow:hidden;box-shadow:0 4px 12px rgba(0,0,0,.04)">
        <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;padding:14px 20px;background:#f8fafc;border-bottom:1px solid #e5e7eb">
            <div style="font-size:14px;font-weight:700;color:#374151;display:flex;align-items:center;gap:8px;margin-right:auto">
                <i class="fa fa-list-alt" style="color:#2563eb;font-size:13px"></i> Leave Schedule
            ...
            <select id="leaveFilter" style="padding:6px 10px;border:1px solid #d1d5db;border-radius:6px;font-size:12px;background:#fff;cursor:pointer"><option value="">All Leave Types</option></select>
        ...
        <div class="table-container" style="height:520px">
            <table style="width:100%;border-collapse:collapse;font-size:14px">
                <thead>
                    <tr style="background:#f1f5f9;position:sticky;top:0;z-index:10">
                        ...
                        <th style="padding:12px 20px;text-align:center;font-size:12px;font-weight:700;color:#374151;text-transform:uppercase;letter-spacing:0.5px;border-bottom:2px solid #e2e8f0;width:100px">Status</th>
                    </tr>
                </thead>
        ...
        <div style="padding:10px 20px;background:#f8fafc;border-top:1px solid #e5e7eb;display:flex;justify-content:space-between;align-items:center;font-size:12px;color:#6b7280">
            <span>Showing ${data.length} leave${data.length!==1?'s':''} — ${on_leave} on leave, ${upcoming} upcoming</span>
