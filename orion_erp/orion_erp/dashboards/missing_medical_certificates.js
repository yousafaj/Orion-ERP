frappe.dom.freeze("Loading Medical Certificate Status...");

async function load_missing_medical_certificates() {
    root_element.innerHTML = "";

    try {
        const { message: data } = await frappe.call({
            method: "orion_erp.api.get_missing_medical_certificates"
        });
...
<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:300px;padding:60px 40px;background:#fff;border-radius:12px;box-shadow:0 2px 12px rgba(0,0,0,.06);font-family:Inter,system-ui,sans-serif">
    ...
    <h3 style="margin:0 0 8px;color:#374151;font-weight:600;font-size:20px">No Missing Certificates</h3>
    <p style="margin:0;color:#9ca3af;font-size:14px">All sick leave applications have medical certificates uploaded.</p>
</div>`;
            frappe.dom.unfreeze();
            return;
        }
        ...
        rows.forEach((r, idx) => {
            ...
            table_rows += `<tr style="background:${idx % 2 === 0 ? '#fef2f2' : '#fee2e2'}">
<td style="padding:8px 16px;font-size:13px;font-weight:500;color:#1f2937;border-bottom:1px solid #f0f0f0;cursor:pointer" class="emp-link" data-name="${esc_emp}"><b>${esc_emp_name}</b><br><span style="color:#64748b;font-size:11px;white-space:nowrap">${esc_emp}</span></td>
...
<span style="display:inline-block;padding:3px 10px;border-radius:20px;font-size:11px;font-weight:700;color:#dc2626;background:#fef2f2;border:1px solid #fecaca"><i class="fa fa-times" style="margin-right:3px"></i> Missing</span>
</td>
<td style="padding:8px 16px;text-align:center;border-bottom:1px solid #f0f0f0">
...
<div class="dashboard-body">
    <div style="background:linear-gradient(135deg,#1e40af,#2563eb,#3b82f6);border-radius:12px 12px 0 0;padding:12px 20px;display:flex;align-items:center;gap:12px;box-shadow:0 4px 16px rgba(37,99,235,0.25)">
        ...
        <div style="flex:1">
            <div style="font-size:16px;font-weight:700;color:#fff;letter-spacing:-0.3px">Missing Medical Certificates</div>
            <div style="font-size:12px;color:rgba(255,255,255,0.75);margin-top:0">Sick Leaves Without Required Documentation</div>
        </div>
        <div class="header-actions">
            <button id="openLeaveBtn" style="display:flex;align-items:center;gap:5px;padding:5px 14px;border:none;border-radius:6px;background:rgba(255,255,255,0.2);color:#fff;font-size:12px;font-weight:600;cursor:pointer;transition:all 0.2s;backdrop-filter:blur(4px);border:1px solid rgba(255,255,255,0.15)">
                <i class="fa fa-external-link" style="font-size:11px"></i> Open Leave Application
            </button>
    ...
    <div class="kpi-grid">
        <div class="kpi-card" style="display:flex;align-items:center;gap:12px;padding:10px 14px;background:#eff6ff;border-radius:10px;border:1px solid #bfdbfe;transition:all 0.25s">
            <div style="display:flex;align-items:center;justify-content:center;width:36px;height:36px;border-radius:8px;background:linear-gradient(135deg,#dbeafe,#eff6ff);color:#2563eb;font-size:15px;flex-shrink:0"><i class="fa fa-ambulance"></i></div>
            <div>
                <div style="font-size:10px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:1px">Total Sick Leaves</div>
                <div style="font-size:22px;font-weight:800;color:#1d4ed8;line-height:1">${kpi.total_sick_leaves}</div>
        ...
        <div class="kpi-card" style="display:flex;align-items:center;gap:12px;padding:10px 14px;background:#fef2f2;border-radius:10px;border:1px solid #fecaca;transition:all 0.25s">
            ...
            <div>
                <div style="font-size:10px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:1px">Missing Certificates</div>
                <div style="font-size:22px;font-weight:800;color:#dc2626;line-height:1">${kpi.missing_certificates}</div>
                <div style="font-size:11px;color:#6b7280;margin-top:0">Not Uploaded</div>
            </div>
        </div>
        <div class="kpi-card" style="display:flex;align-items:center;gap:12px;padding:10px 14px;background:#f0fdf4;border-radius:10px;border:1px solid #bbf7d0;transition:all 0.25s">
            ...
            <div>
                <div style="font-size:10px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:1px">Uploaded</div>
                <div style="font-size:22px;font-weight:800;color:#15803d;line-height:1">${kpi.uploaded_certificates}</div>
                <div style="font-size:11px;color:#6b7280;margin-top:0">With Certificate</div>
            </div>
        </div>
        <div class="kpi-card" style="display:flex;align-items:center;gap:12px;padding:10px 14px;background:#fafafa;border-radius:10px;border:1px solid #e5e7eb;transition:all 0.25s">
            ...
            <div>
                <div style="font-size:10px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:1px">Compliance</div>
                <div style="font-size:22px;font-weight:800;color:#1f2937;line-height:1">${kpi.compliance_pct}%</div>
                <div style="font-size:11px;color:#6b7280;margin-top:0">Certificates Uploaded</div>
            </div>
        </div>
    </div>
    ...
    <div style="background:#fff;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 12px 12px;overflow:hidden;box-shadow:0 4px 12px rgba(0,0,0,.04)">
        <div style="display:flex;align-items:center;padding:10px 16px;background:#f8fafc;border-bottom:1px solid #e5e7eb">
            <div style="font-size:13px;font-weight:700;color:#374151;display:flex;align-items:center;gap:6px;margin-right:auto">
                <i class="fa fa-list-alt" style="color:#2563eb;font-size:12px"></i> Missing Certificates
            ...
            <table style="width:100%;border-collapse:collapse;font-size:13px">
                <thead>
                    <tr style="background:#f1f5f9;position:sticky;top:0;z-index:10">
                        ...
                        <th style="padding:10px 16px;text-align:left;font-size:11px;font-weight:700;color:#374151;text-transform:uppercase;letter-spacing:0.5px;border-bottom:2px solid #e2e8f0">Leave Application</th>
                        ...
                        <th style="padding:10px 16px;text-align:center;font-size:11px;font-weight:700;color:#374151;text-transform:uppercase;letter-spacing:0.5px;border-bottom:2px solid #e2e8f0;width:110px">Medical Certificate</th>
        ...
        <div style="padding:6px 16px;background:#f8fafc;border-top:1px solid #e5e7eb;display:flex;justify-content:space-between;align-items:center;font-size:11px;color:#6b7280">
            <span>Showing ${total_rows} missing certificate${total_rows !== 1 ? 's' : ''}</span>
        ...
        $("#refreshBtn", root_element).on("click", function (e) { e.stopPropagation(); load_missing_medical_certificates(); });
        $("#openLeaveBtn", root_element).on("click", function (e) { e.stopPropagation(); frappe.set_route("List", "Leave Application"); });

        $("#exportBtn", root_element).on("click", function () {
            const today = frappe.datetime.get_today();
            const csv = [];
            csv.push('"MISSING MEDICAL CERTIFICATES"');
            csv.push('');
            csv.push('"Export Date","' + frappe.datetime.str_to_user(today) + '"');
            csv.push('"Total Sick","' + kpi.total_sick_leaves + '","Missing","' + kpi.missing_certificates + '","Compliance","' + kpi.compliance_pct + '%"');
            csv.push('');
            csv.push('');
            const headers = [];
            ...
            link.download = "Missing_Medical_Certificates_" + today + ".csv";
...
<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:300px;padding:60px 40px;background:#fff;border-radius:12px;box-shadow:0 2px 12px rgba(0,0,0,.06);font-family:Inter,system-ui,sans-serif">
    ...
    <button id="retryBtn" style="padding:10px 24px;border:none;border-radius:8px;background:#ef4444;color:#fff;font-size:14px;font-weight:600;cursor:pointer;font-family:inherit">Try Again</button>
        ...
        $("#retryBtn", root_element).on("click", function (e) { e.stopPropagation(); load_missing_medical_certificates(); });
    } finally {
        frappe.dom.unfreeze();
    }
}

load_missing_medical_certificates();
