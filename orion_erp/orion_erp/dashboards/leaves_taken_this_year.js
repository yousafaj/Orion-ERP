frappe.dom.freeze("Loading Leaves Taken This Year...");

async function load_leaves_taken() {
    ...
    try {
        ...
        const { message: leave_data } = await frappe.call({
            method: "orion_erp.api.get_leaves_taken_this_year"
        });
...
<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:300px;padding:60px 40px;background:#fff;border-radius:12px;box-shadow:0 2px 12px rgba(0,0,0,.06);font-family:Inter,system-ui,sans-serif">
    <div style="font-size:64px;margin-bottom:20px;opacity:0.3">&#x1F4CB;</div>
    <h3 style="margin:0 0 8px;color:#374151;font-weight:600;font-size:20px">No Leave Taken This Year</h3>
    <p style="margin:0;color:#9ca3af;font-size:14px">You have not taken any leave so far this year.</p>
</div>`;
        ...
        leave_data.forEach((row, idx) => {
            const days = Number(row.days_taken || 0);
            ...
            table_rows += [
                ...
                '<span style="display:inline-block;padding:4px 16px;border-radius:20px;font-size:13px;font-weight:700;color:#fff;background:linear-gradient(135deg,#16a34a,#15803d);box-shadow:0 2px 6px rgba(22,163,74,0.25);min-width:60px">',
                days, ' Day', (days !== 1 ? 's' : ''),
                '</span></td></tr>'
            ].join('');
        });
...
<div style="font-family:Inter,system-ui,sans-serif;color:#1f2937">
    <div style="background:linear-gradient(135deg,#1e40af,#2563eb,#3b82f6);border-radius:12px 12px 0 0;padding:20px 28px;display:flex;align-items:center;gap:16px;box-shadow:0 4px 16px rgba(37,99,235,0.25)">
        ...
        <div style="flex:1">
            <div style="font-size:20px;font-weight:700;color:#fff;letter-spacing:-0.3px">Leaves Taken This Year</div>
            <div style="font-size:13px;color:rgba(255,255,255,0.75);margin-top:2px">Leave Consumption Summary</div>
        </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;padding:20px 24px;background:#ffffff;border-left:1px solid #e5e7eb;border-right:1px solid #e5e7eb">
        ...
        <div style="display:flex;align-items:center;gap:16px;padding:18px 20px;background:#eff6ff;border-radius:10px;border:1px solid #bfdbfe;transition:all 0.25s">
            ...
            <div>
                <div style="font-size:11px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:2px">Current Leave Year</div>
                <div style="font-size:15px;font-weight:700;color:#1d4ed8;line-height:1.1">${current_year}</div>
                <div style="font-size:12px;color:#6b7280;margin-top:2px">Calendar Year</div>
            </div>
        </div>
        ...
        <div style="display:flex;align-items:center;gap:16px;padding:18px 20px;background:#f0fdf4;border-radius:10px;border:1px solid #bbf7d0;transition:all 0.25s">
            ...
            <div>
                <div style="font-size:11px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:2px">Leave Types Used</div>
                <div style="font-size:28px;font-weight:800;color:#15803d;line-height:1.1">${total_types}</div>
                <div style="font-size:12px;color:#6b7280;margin-top:2px">This Year</div>
            </div>
        </div>

        <div style="display:flex;align-items:center;gap:16px;padding:18px 20px;background:#fafafa;border-radius:10px;border:1px solid #e5e7eb;transition:all 0.25s">
            <div style="display:flex;align-items:center;justify-content:center;width:46px;height:46px;border-radius:10px;background:linear-gradient(135deg,#f3f4f6,#f9fafb);color:#6b7280;font-size:20px;flex-shrink:0"><i class="fa fa-calculator"></i></div>
            <div>
                <div style="font-size:11px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:2px">Total Days Taken</div>
                <div style="font-size:28px;font-weight:800;color:#1f2937;line-height:1.1">${total_days_taken}</div>
                <div style="font-size:12px;color:#6b7280;margin-top:2px">Days Consumed</div>
            </div>
        </div>
    </div>
    ...
    <div style="background:#fff;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 12px 12px;overflow:hidden;box-shadow:0 4px 12px rgba(0,0,0,.04)">
        <div style="display:flex;align-items:center;padding:14px 20px;background:#f8fafc;border-bottom:1px solid #e5e7eb">
            <div style="font-size:14px;font-weight:700;color:#374151;display:flex;align-items:center;gap:8px">
                <i class="fa fa-list-alt" style="color:#2563eb;font-size:13px"></i> Leave Breakdown
            ...
            <table style="width:100%;border-collapse:collapse;font-size:14px">
                <thead>
                    <tr style="background:#f1f5f9;position:sticky;top:0;z-index:2">
                        ...
                        <th style="padding:12px 20px;text-align:center;font-size:12px;font-weight:700;color:#374151;text-transform:uppercase;letter-spacing:0.5px;border-bottom:2px solid #e2e8f0;width:180px">Days Taken</th>
                    </tr>
                </thead>
